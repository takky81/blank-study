...
export function EditorPage() {
  ...
  const [place, setPlace] = useState<MaterialPlace | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      const [found, rows, words] = await Promise.all([
        getMaterialPlace(materialId),
  ...
  useTrail(
    place === null
      ? []
      : [{ label: place.subjectName, to: `/subjects/${place.subjectId}` }, { label: place.name }],
  );
  ...
  if (notFound) {
    return (
      <div className="flex flex-col items-start gap-4 p-10">
        <p className="text-[13px] text-muted">この教材は見つかりませんでした。</p>
  ...
  return (
    ...
    <div className="flex flex-col md:h-[calc(100dvh-3.5rem)] md:overflow-hidden">
      <div className="flex flex-wrap items-center gap-3 border-b-2 border-ink bg-panel px-6 py-2">
        ...
        <span className="grow" />
        {dirty && <span className="text-[12px] text-warn">未保存の変更あり</span>}
        <button
          type="button"
          onClick={() => void save()}
          disabled={!dirty || busy}
        ...
        <div role="tablist" className="flex border-b border-line">
          {(
            ...
            <button
              ...
              className={
                tab === value
                  ? 'h-11 grow border-b-2 border-ink text-[14px]'
                  : 'h-11 grow text-[13px] text-muted'
              }
            >
              {label}
            </button>
      ...
      {error !== '' && (
        <div
          role="alert"
          className="m-4 flex items-center gap-3 rounded border-2 border-warn bg-warn-panel p-3 text-[14px] text-warn"
        >
      ...
      <div className="flex min-h-[calc(100dvh-8rem)] flex-col md:min-h-0 md:grow md:flex-row">
              ...
              {creating && (
                ...
                >
                  ...
                  <div className="flex gap-2">
                    ...
                    <button type="button" onClick={() => setCreating(false)} className="btn-s">
                      やめる
                    </button>
                  </div>
                </form>
        ...
        {selected === null ? (
          <section className="flex grow items-center justify-center bg-panel p-4">
            ...
            <div className="flex shrink-0 flex-wrap items-center gap-2 border-b-[1.5px] border-line bg-subtle px-4 py-2">
              {renaming ? (
                ...
                >
                  ...
                  <button type="button" onClick={() => setRenaming(false)} className="btn-s">
                    やめる
                  </button>
                </form>
              ) : (
                <>
                  <h2 className="grow text-[15px]">{selected.title}</h2>
                  <button
                    type="button"
            ...
            <div className="flex min-h-0 grow flex-col lg:flex-row">
              {showEdit && (
                <div className="flex min-h-0 min-w-0 grow flex-col border-ink lg:basis-0 lg:border-r-2">
                  ...
                  />
                  <p className="pane-ft">章の2回目以降の出現は id のみで表示される</p>
                </div>
              )}
              {showPreview && (
